 ...
 * Business Pro Theme
 *
 * This file adds the page header functionality to the Business Pro theme.
 *
...
add_action( 'genesis_before', 'business_page_header_setup' );
/**
 * Set up page header.
 *
 ...
 * @return void
 */
function business_page_header_setup() {
	...
	add_action( 'business_page_header', 'genesis_do_posts_page_heading' );
	add_action( 'business_page_header', 'genesis_do_date_archive_title' );
	add_action( 'business_page_header', 'genesis_do_taxonomy_title_description' );
	add_action( 'business_page_header', 'genesis_do_author_title_description' );
	add_action( 'business_page_header', 'genesis_do_cpt_archive_title_description' );
...
add_action( 'business_page_header', 'business_page_title', 10 );
/**
...
function business_page_title() {
	...
	if ( class_exists( 'WooCommerce' ) && is_search() ) {
		...
		genesis_markup( array(
			'open'    => '<h1>' . apply_filters( 'genesis_search_title_text', __( 'Search results for: ', 'business-pro-theme' ) ),
			'close'   => '</h1>',
...
add_action( 'business_page_header', 'business_page_excerpt', 20 );
/**
...
add_action( 'genesis_before_content_sidebar_wrap', 'business_page_header' );
/**
 * Display the page header.
 *
 * Conditionally outputs the opening and closing page header markup and runs
 * business_page_header which all of our header functions are hooked to.
 *
 * @since  1.0.5
 *
 * @return void
 */
function business_page_header() {

	// Output opening markup.
	if ( apply_filters( 'business_page_header_open', true ) ) {
	 ...
	 */
	do_action( 'business_page_header' );

	// Output closing markup.
	if ( apply_filters( 'business_page_header_close', true ) ) {
